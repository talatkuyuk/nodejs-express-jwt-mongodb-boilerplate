...
const querystrings = [
	"size=3&page=2&email=xxx@xxx.com",
	"size=3&page=2",
	"",
]
...
describe('Utils pick function', () => {
  it('picks the keys from request query', () => {
    expect(Utils.pick(querystring.parse(querystrings[0]), ['email'])).toEqual({ "email": "xxx@xxx.com"} );
	expect(Utils.pick(querystring.parse(querystrings[0]), ['email', 'size'])).toEqual({ email: "xxx@xxx.com", size: "3" });
