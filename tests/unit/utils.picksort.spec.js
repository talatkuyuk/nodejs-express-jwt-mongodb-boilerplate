...
const querystrings = [
	...
	{ input: "size=3&page=2&size=4&sort=email.desc|id.asc|isDisabled", output: { email: -1, id: 1, isDisabled: 1, createdAt: -1 } }
]
  ...
  it('picks the sort key from the request query', () => {
	  for (const query of querystrings) {
		console.log(query.input)
		  ...
		  expect(Utils.pickSort(querySort, ['id', 'email', 'isEmailVerified', 'isDisabled', 'createdAt'])).toEqual(expected);
	  }
  });
});